...
sub new {
  ...
  return bless {
    ...
    spampd => {
      ...
      user_dir          => '',                    # Root directory for per-user settings
    },
...
Options:
  ...
  --user-dir <path>          Use the path as the root for SA per-user configuration
